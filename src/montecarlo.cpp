...
std::pair<char , std::size_t> chooseRandomResidue(const std::unique_ptr<PDBStructure>& structure)
{
    ...
    std::uniform_int_distribution<> dist(0, structure->interfaceResidues.size() - 1); // for chain selection
    auto chain = structure->interfaceResidues.begin();
    if (!structure->interfaceResidues.empty()) {
        while(resindex == std::numeric_limits<std::size_t>::max()) {
            chain = structure->interfaceResidues.begin();
            std::advance(chain, dist(thread_rng));
            if (chain != structure->interfaceResidues.end() && !chain->second.empty()) {
...
double rotateResidueSidechainRandomly(std::unique_ptr<PDBStructure> & structure,  char& chain, std::size_t resNum, bool verbose)
{

    if (verbose)
    {
        std::cout <<  std::endl << "Size of chain: " << structure->chains.at(chain).size() << std::endl << "Trying to perturb..." << std::endl;
    }
    ...
    Residue ref_res(structure->chains.at(chain).at(resNum));
    std::string resName = structure->chains.at(chain).at(resNum).resName;
    ref_res = structure->chains.at(chain).at(resNum);
    ...
    if(resName!="GLY" && resName!= "PRO" && resName!= "ALA" && amino_acids::axes::AMINO_MAP.find(resName) != amino_acids::axes::AMINO_MAP.end() ) {
        for (const std::string &axis: amino_acids::axes::AMINO_MAP.at(resName)) {
            ...
            std::vector<std::string> sub_atoms = std::vector<std::string>(first, amino_acids::atoms::AMINO_MAP.at(
                    resName).end());
            std::valarray<double> rot_coords = findRotationAxis(structure->chains.at(chain).at(resNum), axis);
            double vec_norm = std::sqrt(std::pow(rot_coords, 2).sum());
            double angle = dist(thread_rng);

            for (Atom& atom: structure->chains.at(chain).at(resNum).atoms)
            ...
            auto local_distance_matrix = structure->calculateLocalDistanceMatrix(structure->chains.at(chain).at(resNum));

            if (detect_clashes(local_distance_matrix, 0.21)) {
                if (verbose) std::cout << "Atoms clashed, retrying..." << std::endl;
                structure->chains.at(chain).at(resNum) = ref_res;
                angles = angles / 10;
                std::uniform_real_distribution<double> dist(-angles, angles);
            } else structure->updateDistanceMatrixLocally(local_distance_matrix, chain, resNum); // update distance matrix

            rmsd = calculateRMSD(ref_res.atoms, structure->chains.at(chain).at(resNum).atoms);
